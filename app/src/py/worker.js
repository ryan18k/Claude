...
async function init() {
  ...
  py = await loadPyodide({indexURL: BASE, stdLibURL: std});
  ...
  return {ms: Date.now() - t0, version: py.runPython('sympy.__version__')};
}
...
self.onmessage = async (e) => {
  ...
  try {
    ...
    self.postMessage({id, ok: false, error: String((err && err.message) || err).slice(-600)});
  }
};
